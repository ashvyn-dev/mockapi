...
def generate_openapi_schema(collection) -> str:
    ...
    endpoints = collection.endpoints.filter(is_active=True).order_by('path', 'http_method')
    ...
    for endpoint in endpoints:
        ...
        operation = {
            ...
            "description": endpoint.description or f"{endpoint.http_method} {endpoint.display_name}",
            "responses": {}
        }
        ...
        operation["responses"][str(endpoint.response_status)] = {
            ...
            "content": {
                endpoint.content_type: {
                    "example": _parse_response_body(endpoint.response_body, endpoint.content_type)
                }
            }
        }
            ...
            operation["responses"][str(response.response_status)] = {
                ...
                "content": {
                    response.content_type: {
                        "example": _parse_response_body(response.response_body, response.content_type)
                    }
                }
            }
            ...
            for path, methods in schema["paths"].items():
                ...
                for method, operation in methods.items():
                    ...
                    if created:
                        ...
                        endpoint.display_name = operation.get("summary", endpoint.display_name)
                        endpoint.description = operation.get("description", endpoint.description)
                        updated_count += 1
                    
                    # Get response info from first response
                    if "responses" in operation:
                        for status_code, response_data in operation["responses"].items():
                            try:
                                ...
                                if "content" in response_data:
                                    for content_type, content_data in response_data["content"].items():
                                        ...
                                        if "example" in content_data:
                                            import json
                                            if content_type == "application/json":
                                                endpoint.response_body = json.dumps(content_data["example"], indent=2)
                                            else:
                                                endpoint.response_body = str(content_data["example"])
                                        break
                                break
